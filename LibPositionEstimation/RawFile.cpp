...
bool CRawFile::FileOpen( CString *pstrFilename, int iMode )
{
    return FileOpen( (LPSTR)(LPCTSTR) (*pstrFilename), iMode );
...
{
	unsigned long long int file_byte=0;

// 	if( m_fid != NULL ) {
//         file_byte = _lseek( m_fid, 0L, SEEK_END )
// 		_lseek( m_fid, 0L, SEEK_SET );
// 
// 	}

    file_byte = GetFileSize( m_fullname );
